...
- **One dispatch per day is structural, not scheduled.** The handler's first act is a `dispatch.claimed` event keyed `x.dispatch:<YYYY-MM-DD>`; `appendEvent`'s unique dedupe index makes a second claim a no-op. A double-fired cron, a manual POST, and a reclaimed job all collapse to one run. Admin review runs claim a `manual:<ms>` suffixed slot so they never consume the day.
...
- **A failed post is a skip, never a retry** (`post_failed`). A retry that succeeds after a timeout has already posted.
