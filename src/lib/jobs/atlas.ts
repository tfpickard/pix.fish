import { enqueueIfNonePending } from '@/lib/db/queries/jobs';
 ...
 * all. Four call sites today: persistEnrichment, the reprocess handler,
 * scripts/backfill-embeddings.ts, and DELETE /api/images/[slug] -- a delete
 * removes a vector, which counts.
 *
 ...
 *
 * The dedupe is atomic (see enqueueIfNonePending). It has to be: the upload UI
 * fires N uploads in parallel, so the burst this debounce exists to collapse
 * is precisely the burst a check-then-insert race would let through.
 *
