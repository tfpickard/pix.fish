...
export function FishStatsBanner({ stats, fishHidden, onShowFish }: FishStatsBannerProps) {
  ...
  const barClass =
    'fixed inset-x-0 top-0 z-30 flex flex-wrap items-center gap-x-4 gap-y-1 border-b border-ink-800/70 bg-ink-950/80 px-3 py-1.5 font-mono text-[11px] text-ink-300 backdrop-blur';
  ...
  if (hidden) {
    return (
      <button
        ...
        className="fixed right-3 top-2 z-30 rounded-full border border-ink-800/70 bg-ink-950/80 px-2 py-0.5 font-mono text-[10px] text-ink-500 backdrop-blur transition-colors hover:text-ink-200"
      >
        ▾ stats
      </button>
    );
