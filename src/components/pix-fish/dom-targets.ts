...
function visibleRectsIn(viewport: { w: number; h: number }, selector: string): DOMRect[] {
...
export function pickPerchTarget(): PerchTarget | null {
  ...
  const rects = visibleRectsIn(viewport, PERCH_SELECTORS);
...
export function pickHideTarget(): PerchTarget | null {
  const viewport = { w: window.innerWidth, h: window.innerHeight };
  const rects = visibleRectsIn(viewport, HIDE_SELECTORS);
  if (rects.length === 0) return null;
  const rect = rects[Math.floor(Math.random() * rects.length)];
